...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer player, float p_187457_2_, float p_187457_3_, EnumHand p_187457_4_, float p_187457_5_, ItemStack p_187457_6_, float p_187457_7_) {
        if (mc.player.isPlayerBlocking) {
            if (!mc.gameSettings.keyBindUseItem.isKeyDown()) {
                mc.player.isPlayerBlocking = false;
            }
        }
        float f = 1.0F - (this.prevEquippedProgressMainHand + (this.equippedProgressMainHand - this.prevEquippedProgressMainHand) * p_187457_7_);
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.isEmpty()) {
                ...
                if (player.getItemInUseCount() > 0 && !mc.player.isPlayerBlocking) {
                    int j = flag1 ? 1 : -1;
                ...
                } else if (player.getHeldItemMainhand().getItem() instanceof ItemSword && this.mc.player.isPlayerBlocking && mc.gameSettings.keyBindUseItem.isKeyDown()) {
                    GlStateManager.translate(Animations.xValue.get(), Animations.yValue.get(), Animations.zValue.get());

                    if (Animations.Sword.get() == "WindMill")  {
                        WindMill(-0.2F + p_187457_7_, p_187457_5_);
                        doItemRenderGLTranslate();
                    } else if (Animations.Sword.get() == "Avatar") {
                        avatar(p_187457_5_);
                        doBlockTransformations();
                    } else if (Animations.Sword.get() == "Push") {
                        push(p_187457_5_);
                        doBlockTransformations();
                    } else if (Animations.Sword.get() == "ETB") {
                        etb(p_187457_7_,p_187457_5_);
                        doBlockTransformations();
                    } else if (Animations.Sword.get() == "Min") {
                        min(1f,p_187457_7_,p_187457_5_);
                    }
                    ...
                    float v = -0.4F * MathHelper.sin(MathHelper.sqrt(p_187457_5_) * (float) Math.PI);
                    float v1 = 0.2F * MathHelper.sin(MathHelper.sqrt(p_187457_5_) * ((float) Math.PI * 2F));
                    float v2 = -0.2F * MathHelper.sin(p_187457_5_ * (float) Math.PI);
                    int i = flag1 ? 1 : -1;
                    GlStateManager.translate((float) i * v, v1, v2);
                    this.transformSideFirstPerson(enumhandside, p_187457_7_);
                    this.transformFirstPerson(enumhandside, p_187457_5_);
                }

                this.renderItemSide(player, p_187457_6_, flag1 ? ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND : ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, !flag1);
            }
    ...
    private void push(float idc) {
        ...
        float var4 = MathHelper.sin(MathHelper.sqrt(idc) * 3.1415927F);
    ...
    private void WindMill(float equippedProg, float swingProgress) {
        doItemRenderGLTranslate();
        GlStateManager.translate(0.56, -0.52 + equippedProg * -0.6, -0.72);
        GlStateManager.translate(-0.1414214, 0.08, 0.1414214);
        GlStateManager.rotate(-102.25F, 1, 0, 0);
        GlStateManager.rotate(13.365F, 0, 1, 0);
        GlStateManager.rotate(78.050003F, 0, 0, 1);
        double f = Math.sin(swingProgress * swingProgress * Math.PI);
        double f1 = Math.sin(Math.sqrt(swingProgress) * Math.PI);
        GlStateManager.rotate((float) (f * 31.3), 0, 1, 0);
        GlStateManager.rotate((float) (f1 * 19.13), 0, 0, 1);
        GlStateManager.rotate((float) (f1 * -47.83), 1, 0, 0);
        doItemRenderGLScale();
    }
