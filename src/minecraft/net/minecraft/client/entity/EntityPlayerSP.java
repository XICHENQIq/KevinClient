...
import static kevin.utils.PlayerExtensionKt.getNearestPointBB;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public double getDistanceToEntityBox(Entity entity) {
        Vec3d eyes = this.getPositionEyes(1F);
        Vec3d pos = getNearestPointBB(eyes, entity.boundingBox);
        double xDist = MathHelper.abs(pos.x - eyes.x);
        double yDist = MathHelper.abs(pos.y - eyes.y);
        double zDist = MathHelper.abs(pos.z - eyes.z);
        return MathHelper.sqrt(xDist + yDist + zDist);
    }
    ...
    public void onLivingUpdate()
    {
        ...
        // fixed
        final DuelGod duelGod = (DuelGod) KevinClient.moduleManager.getModule(DuelGod.class);
        if (duelGod.getState()) {
        ...
        if (this.isSprinting()) {
            sprint.getState();
            if (this.movementInput.moveForward < 0.8F || this.isCollidedHorizontally || !flag4) {
                this.setSprinting(false);
            }
        }
