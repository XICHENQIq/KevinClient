...
public class FloatButton extends Button {
    ...
    public void drawButton(int x, int y, int mx, int my, float pticks, float alpha) {
        ...
        FontLoader.getCFont(false,20).drawString(value.getName() + ":", x, y, new Color(255, 255, 255, intalpha).getRGB());
        int dx = x + 195;
        int tx = x + 295;
        ...
        FontLoader.getCFont(false,20).drawString(value.get() + "", tx + 7, y, -1);
        ...
        if(ClickUtils.isClickable(dx, y + 3 - 2, tx, y + 5 + 2, mx, my) && Mouse.isButtonDown(0)){
            ...
            pp = MathHelper.clamp(pp, 0.0, 1.0);
