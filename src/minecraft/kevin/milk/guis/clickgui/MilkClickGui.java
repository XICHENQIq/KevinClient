package kevin.milk.guis.clickgui;
...
import kevin.milk.utils.render.anims.AnimationUtils;
...
public class MilkClickGui extends GuiScreen {
...
//        Color white2 = new Color(60, 60, 60, animAlpha);
        // title
        FontLoader.getCFont(true, 24).drawString("Kevin", x - 34, y + 12, white.getRGB());
        FontLoader.getCFont(false, 16).drawString(KevinClient.INSTANCE.getVersion(), x - 8 + 5, y + 7, white.getRGB());
        ...
        for (ModuleCategory category : ModuleCategory.getEntries()) {
            ...
            if (category == selectCategory) {
                ...
                animForCategory2 = (float) animationUtils4.animate(FontLoader.getCFont(false, 20).getStringWidth(category.name() + 10), animForCategory2, 0.2);
...
//                RenderUtils.drawRect(x - 34, y + animForCategory, x - 40 + animForCategory2, y + animForCategory + 13, rainbow.getRGB());
                // category
                FontLoader.getCFont(false, 18).drawString(category.name(), x - 33, categoryY + 5, RenderUtils.skyRainbow(1, 1f, 1f).getRGB());
                // for all module
                if (selectPanel == null) {
                    StencilUtil.initStencilToWrite();
                    Gui.drawRect(x + 50, y, x + 400, y + 300, -1);
...
//                        Color nameColor = ColorUtils.blend(rainbow, new Color(224, 224, 224), m.anim);
                        FontLoader.getCFont(false, 22).drawString(m.getName(), x + 66 + categoryX, moduleY + 8, ColorUtils.reAlpha((m.getState() ? Color.GREEN.brighter() : Color.RED.brighter()), animAlpha).getRGB());
                        FontLoader.getCFont(false, 18).drawString(m.getDescription(), x + 66 + categoryX, moduleY + 23, new Color(95, 93, 101, animAlpha).getRGB());
                        moduleY += 45;
                    }
                    StencilUtil.uninitStencilBuffer();
                }
            } else {
                FontLoader.getCFont(false, 18).drawString(category.name(), x - 33, categoryY + 5, -1);
            }
            categoryY += 20;
        }
        categoryY += 20;
        FontLoader.getCFont(false, 18).drawString("CONFIG", x - 33, categoryY + 5, -1);
