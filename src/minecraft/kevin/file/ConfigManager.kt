...
object ConfigManager {
    ...
    fun removeConfig(name: String): Int {
        val configFile = File(configDir, "$name.json")
        val hudFile = File(configDir, "$name-HUD.json")
        var returnValue = 0 // 0 indicates success
        if (configFile.exists() && configFile.isFile) {
            if (configFile.delete()) {
                // Remove config from the list if it exists
                configList.removeIf { it.name == "$name.json" }
            } else {
                returnValue = 1 // 1 indicates failure to delete the config file
            }
        }
        if (hudFile.exists() && hudFile.isFile) {
            if (!hudFile.delete() && returnValue == 0) {
                returnValue = 2 // 2 indicates failure to delete the HUD file, but config file was deleted successfully
            }
        }
        // Possible returnValue states:
        // 0 - Both files deleted or non-existent
        // 1 - Failed to delete main config file
        // 2 - Failed to delete HUD file but main config file was deleted
        return returnValue
    }
