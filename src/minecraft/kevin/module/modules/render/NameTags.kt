...
class NameTags : Module("NameTags", "Changes the scale of the nametags so you can always read them.", category = ModuleCategory.RENDER) {
    ...
    private val fontValue = FontLoader.getCFont(true,20)
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        when(modeValue.get().toLowerCase()) {
            "simple" -> {
                ...
                drawRect(-width - 6F, -fontRenderer.FONT_HEIGHT*1.7F, width + 6F, -2F, Color(0,0,0,jelloAlphaValue.get()).rgb)
                ...
                fontRenderer.drawString(tag, (-fontRenderer.getStringWidth(tag)*0.5F), (-fontRenderer.FONT_HEIGHT*1.4F),Color.WHITE.rgb)
            }
            ...
            "jello" -> {
                ...
                //render bg
                glScalef(-scale*2, -scale*2, scale*2)
                drawRect(-width - 4F, -fontRenderer.FONT_HEIGHT*3F, width + 4F, -3F, bgColor.rgb)
                ...
                //string
                fontRenderer.drawString(tag,-width.toFloat(),-fontRenderer.FONT_HEIGHT*2-4F,Color.WHITE.rgb)
                glScalef(0.5F,0.5F,0.5F)
                fontRenderer.drawString("Health: "+entity.health.toInt(),-width*2F, -fontRenderer.FONT_HEIGHT*2F,Color.WHITE.rgb)
            }
        }
        // Reset caps
