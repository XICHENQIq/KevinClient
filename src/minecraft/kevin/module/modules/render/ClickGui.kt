...
class ClickGui : Module("ClickGui","Opens the ClickGUI.", category = ModuleCategory.RENDER, keyBind = Keyboard.KEY_RSHIFT) {
    ...
    private val mode = ListValue("Mode", arrayOf("New","Old","Milk"),"New")

    @EventTarget(ignoreCondition = true)
    fun onPacket(event: PacketEvent) {
        val packet = event.packet
        if (packet is SPacketCloseWindow && (mc.currentScreen is ClickGUI || mc.currentScreen is NewClickGui || mc.currentScreen is MilkClickGui)) {
            event.cancelEvent()
        }
    }
