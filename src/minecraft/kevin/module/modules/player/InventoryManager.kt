...
class InventoryManager : Module(name = "InventoryManager", description = "Automatically throws away useless items.", category = ModuleCategory.PLAYER) {
    ...
    private val elytra = BooleanValue("Elytra", true)
        ...
        return try {
            ...
            if (item is ItemSword || item is ItemTool) {
                ...
                items().none { (slot, stack) ->
                    if (stack != itemStack && stack.item is ItemArmor) {
                        ...
                        if (armor.armorType != currArmor.armorType)
                            ...
                            ARMOR_COMPARATOR.compare(currArmor, armor) <= 0
                    } else
                        false
                }
                ...
                items(0, 45).none { (_, stack) -> itemStack != stack && stack.unlocalizedName == "item.compass" }
                    ...
                    (expValue.get() && item is ItemExpBottle) || item is ItemEgg || item is ItemSnowball
        } catch (ex: Exception) {
            true
        }
    }
    ...
    fun onRender3D(event: UpdateEvent) {
        if (!InventoryUtils.CLICK_TIMER.hasTimePassed(delay2) || mc.player == null ||
                (mc.player.openContainer != null && mc.player.openContainer.windowId != 0)) return

        // Find best armor
        val armorPieces = IntStream.range(0, 36)
                .filter { i: Int ->
                    val itemStack = mc.player.inventory.getStackInSlot(i)
                    (!itemStack.isEmpty && itemStack.item is ItemArmor
                            && (i < 9 || System.currentTimeMillis() - itemStack.itemDelay >= itemDelayValue.get()))
                }
                .mapToObj { i: Int -> ArmorPiece(mc.player.inventory.getStackInSlot(i), i) }
        ...
        // Swap armor
        for (i in 0..3) {
            var armorPiece = bestArmor[i]

            val armorSlot = i

            val oldArmor = ArmorPiece(mc.player.inventory.armorItemInSlot(armorSlot), -1)

            if (elytra.get() && (oldArmor.itemStack.item is ItemElytra || isElytra)) {
                if (!oldArmor.itemStack.isEmpty && ItemElytra.isUsable(oldArmor.itemStack)) continue
                else {
                    val s = IntStream.range(0, 36).filter { v: Int ->
                        val itemStack = mc.player.inventory.getStackInSlot(v)
                        (!itemStack.isEmpty && itemStack.item is ItemElytra
                                && ItemElytra.isUsable(itemStack))
                    }.toArray()
                    isElytra = !isElytra
                    if (s.size != 0) {
                        armorPiece = ArmorPiece(mc.player.inventory.getStackInSlot(s[0]), s[0])
                    }
                }
            }
            ...
            if (ItemUtils.isStackEmpty(oldArmor.itemStack) || oldArmor.itemStack.item !is ItemArmor || ARMOR_COMPARATOR.compare(oldArmor, armorPiece) < 0) {
                if (!ItemUtils.isStackEmpty(oldArmor.itemStack) && move(8 - armorSlot, true)) {
                    locked = true
                    return
                }

                if (ItemUtils.isStackEmpty(mc.player.inventory.armorItemInSlot(armorSlot)) && move(armorPiece.slot, false)) {
                    locked = true
                    return
                }
            }
    ...
    private fun move(item: Int, isArmorSlot: Boolean): Boolean {
        if (!isArmorSlot && item < 9 && hotbarValue.get() && mc.currentScreen !is GuiInventory) {
            ...
            delay2 = TimeUtils.randomDelay(minDelayValue.get(), maxDelayValue.get())
            ...
            if (openInventory) mc.connection!!.sendPacket(CPacketEntityAction(mc.player, CPacketEntityAction.Action.OPEN_INVENTORY))

            var full = isArmorSlot

            if (full) {
                for (iItemStack in mc.player.inventory.mainInventory) {
            ...
            if (full) {
                mc.playerController.windowClick(mc.player.inventoryContainer.windowId, item, 1, ClickType.THROW, mc.player)
            } else {
                mc.playerController.windowClick(mc.player.inventoryContainer.windowId, (if (isArmorSlot) item else (if (item < 9) item + 36 else item)), 0, ClickType.QUICK_MOVE, mc.player)
            }
    ...
    companion object {
        val ARMOR_COMPARATOR: ArmorComparator2 = ArmorComparator2()
    }
}