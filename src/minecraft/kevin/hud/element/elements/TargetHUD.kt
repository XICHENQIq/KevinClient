...
class TargetHUD : Element() {
    ...
    private val mode = ListValue("Mode", arrayOf("Liquid", "Kevin", "Blur","Kevin-New"), "Kevin")
    ...
    override fun drawElement(): Border? {
        var target = (KevinClient.moduleManager.getModule("KillAura") as KillAura).discoverTarget
        ...
        when (mode.get()) {
            ...
            "Kevin-New" -> {
                ...
                val width = (38 + (FontLoader.getCFont(true, 18).getStringWidth(target.displayName.unformattedText)))
                        .coerceAtLeast(118)
                        .toFloat()

                RenderUtils.drawRoundedRect(- 3f, - 3f,
                        width + 3f, 36.0f + 3f , 3.0f,
                        Color(0, 0, 0, 100).rgb,true)
                ...
                if (easingHealth < target.health)
                    RenderUtils.drawRect((easingHealth / target.maxHealth) * width, 34F,
                            (target.health / target.maxHealth) * width, 36F, Color(44, 201, 144).rgb)
                FontLoader.getCFont(true, 20).drawString(target.displayName.unformattedText, 36f, 3f, 0xffffff)
                FontLoader.getCFont(true, 18).drawString("HurtTime: ${decimalFormat.format(target.hurtTime)}", 36f, 15f, 0xffffff)
                // Draw info
                val playerInfo = mc.connection!!.getPlayerInfo(target.uniqueID)
                if (playerInfo != null) {
                    FontLoader.getCFont(true, 18).drawString(
                            if (mc.player.health > target.health) "Win" else "Lose",
                            36f, 24f, 0xffffff)
                    // Draw head
                    val locationSkin = playerInfo.locationSkin
                    drawHead(locationSkin, 30, 30)
            ...
            "Blur" -> {
                ...
                val width = (38 + FontLoader.getCFont(true, 18).getStringWidth(target.name))
                        .coerceAtLeast(118)
                        .toFloat()

                RenderUtils.drawRoundedRect(- 3f,- 3f,width + 3f,36f + 3f ,3f,Color(0,0,0,70).rgb,true)
                GL11.glTranslated(-renderX * scale, -renderY * scale, 0.0)

                BlurBuffer.drawRoundRectBlur(0f - 3f + renderX.toFloat(), 0f - 3f + renderY.toFloat(),
                    width + 3f + renderX.toFloat(), 36f + 3f + renderY.toFloat(), 3f,scale)
                ...
                if (easingHealth < target.health)
                    RenderUtils.drawRect((easingHealth / target.maxHealth) * width, 34F,
                            (target.health / target.maxHealth) * width, 36F, Color(44, 201, 144).rgb)
                FontLoader.getCFont(true, 20).drawString(target.name, 36f, 3f, Color.WHITE.rgb)
                FontLoader.getCFont(true, 18).drawString("HurtTime: ${target.hurtTime}", 36f, 15f, Color.WHITE.rgb)
                // Draw info
                val playerInfo = mc.connection!!.getPlayerInfo(target.uniqueID)
                if (playerInfo != null) {
                    FontLoader.getCFont(true, 18).drawString(
            ...
            "Kevin" -> {
                if (target != null) {
                    ...
                    RenderUtils.drawRoundedRect(-8.5F, -12.5F, 14.75F + x2, 54.5F, 4f, Color(0, 0, 0, 150).rgb, false)
