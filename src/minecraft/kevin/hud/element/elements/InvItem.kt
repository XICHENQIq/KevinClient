...
class InvItem(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F) : Element(x, y, scale){
    ...
    private fun drawRects() {
        RenderUtils.drawRoundedRect(0F,startY,174F,getY2(),4f,Color(0,0,0,80).rgb,true)
        GL11.glTranslated(-renderX, -renderY, 0.0)

        BlurBuffer.drawRoundRectBlur(renderX.toFloat(), startY + renderY.toFloat(),
            174F + renderX.toFloat(), getY2() + renderY.toFloat(), 4f,scale)
        ...
        if (titleRainbow.get()) {
            RainbowFontShader.begin(true, 1.0F / 1000, 1.0F / 1000, System.currentTimeMillis() % 10000 / 10000F).use {
                FontLoader.getCFont(false,18).drawString(
                    ...
                    -(FontLoader.getCFont(false,18).FONT_HEIGHT).toFloat(),
                    0,
                    false
                )
            }
